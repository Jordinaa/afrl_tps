...
class Visualiser(FlightEnvelopeAssessment):
    ...
    def plot_init_vn(self):
        ...
        self.ax.axhline(y=1, color='green', linestyle='--', alpha=0.2, linewidth=.5)
        self.ax.axhline(y=-1, color='green', linestyle='--', alpha=0.2, linewidth=.5)
    ...
    def static_plot(self, static_velocity, static_load_factors):
        ...
        line_labels = ['$Cl_{Max}$', '$Cl_{Max}\cdot$0.938', '$Cl_{Max}\cdot$0.875', '$Cl_{Max}\cdot$0.813', '$Cl_{Max}\cdot$0.750']  # Define the labels for each line   
        line_colors = ['red', 'red', 'red', 'red', 'red']
        line_opac = [1, 0.8, 0.6, 0.4, 0.2]
        for load_factor, line_style, label, line_color, opac in zip(static_load_factors, line_styles, line_labels, line_colors, line_opac):
            self.ax.plot(static_velocity, load_factor, color=line_color, linestyle=line_style, label=label, alpha=opac, linewidth=1)
